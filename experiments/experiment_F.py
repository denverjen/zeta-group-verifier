...
states = ['q_init', 'q_gen', 'q_imag', 'q_verify', 'q_skip', 'q_clear', 'q_inc', 'q_loop', 'qH']
symbols = ['0','.','5',',','#','1','B']

transitions = {}
transitions[('q_init', 'B')] = ('q_gen', '1', 'R')
# Generate "#0.5,1...1" (imag part = counter string)
transitions[('q_gen', 'B')] = ('q_imag', '#', 'R')
transitions[('q_imag', '#')] = ('q_imag', '0', 'R')
transitions[('q_imag', '0')] = ('q_imag', '.', 'R')
transitions[('q_imag', '.')] = ('q_imag', '5', 'R')
transitions[('q_imag', '5')] = ('q_imag', ',', 'R')
transitions[('q_imag', ',')] = ('q_verify', '1', 'R')   # one '1' as imag part
# Verify
...
print("===== Experiment F: Asymptotic zero-generation formula =====")
print(f"Generators: {len(generators)}")
print(f"Abelianization image of w == 0? {result}  => w {'=' if result else '≠'} 1")
if not result:
    print("Conclusion: Machine never halts (RH true for this family).")
else:
